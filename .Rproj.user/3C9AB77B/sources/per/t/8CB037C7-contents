...
schoolwise <- function(dataset_name="knownprograms2001_2015",
    ...
    # 4a. base names (we'll later add clust method and .pdf or .tiff to the ends)
    ...
    for (fun_name in myclustfun) {
        ...
        filename <- paste0(filenamebase, " -- ", fun_name, filename_suffix, filetype)
...
method_spread_across_schools <- function(dataset_name = "knownprograms2001_2015",
    ...
    if(do.plot) {
        ...
        p <- barplot(if (pcts) my.pcts else to.plot,
                     ...
                     ylab = if(horizontal == F) paste0(if(pcts) {"Percentage of programs"} else {"Number of programs"}),
                     xlab = if(horizontal == T) paste0(if(pcts) {"Percentage of programs"} else {"Number of programs"})
        ...
        # add percentage values before the start of each label
...
if (FALSE) {
    ...
    if(remake_figs) {
        ...
        outfile <- paste0("method_count-v-diss_count--", dataset_name,
                          "--", tagset_name, filetype)
        outfile <- file.path(imageloc, outfile)
        pdf(outfile)
        # tiff(outfile, height=2400, width=2400, units="px", res=400, compression="lzw")
    }
