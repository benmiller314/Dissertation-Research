...
build_plot_title <- function(dataset_name, ntopics, iter_index, subset_name=NULL,
    ...
    if(! is.null(ntopics) && is.numeric(ntopics)) { ntopics_part <- paste0("k", ntopics)
    } else { ntopics_part <- ""}

    if(length(bad.topics > 0)) { bad_topics_part <- paste0(", ", length(bad.topics), " bad topics hidden")
    } else { bad_topics_part <- "" }

    if(!is.null(iter_index) && iter_index > 0) { iter_part <- paste0("_iter", iter_index)
    } else { iter_part <- "" }
    ...
    main <- paste0(whatitis, ", ",                                       # type of plot/file
                   dataset_name, ntopics_part, iter_part,                # which topic model
