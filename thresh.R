...
thresh <- function(dataset_name="noexcludes", tagset_name="tagnames", threshold=5, since=2006, until=2010) {
	...
	thresh.report <- paste0("Found ", length(d3), " schools (out of ", nrow(d2), " schools in ", dataset_name, " from ", since, "-", until, ") with ", threshold, " or more dissertations.")

	## 2. get full 10-year tag data for those schools
	d4 <- dataset[which(dataset$School %in% d3),]
	
	return(list("thresh.data"=d4, "thresh.report"=thresh.report))
}
